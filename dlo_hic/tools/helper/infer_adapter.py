...
@click.option("--prob-thresh", default=0.5,
...
def _main(fq_path, n, prob_thresh, figure, fig_size, dpi):
    """
    Inference adapter sequence, and plot the decision graph.
    """
    probs, flag_seq, adapter_seq = infer_adapter(fq_path, n, prob_thresh)
    print("Flag sequence:", file=sys.stderr)
    print(flag_seq, file=sys.stderr)
    print("Adapter:", file=sys.stderr)
    print(adapter_seq)
    fig, _ = plot_char_prob_stacked_bar(probs)
